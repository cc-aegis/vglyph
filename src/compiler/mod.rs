...
pub fn compile(src: &str) -> HashMap<String, (Vec<String>, Vec<Line>)> {
    ...
    while let Some(mut next) = parser.next() {
        let input = next.get_input(&inputs);
        inputs.insert(next.names[0].clone(), input);
